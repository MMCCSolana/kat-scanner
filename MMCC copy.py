...
def app():
    ...
    #api links 
    reward_url = 'https://api.solscan.io/account?address=FLARQdNnEz8qzGD5oS67eZbSmY94dqx5RheRXJmHjsaX'
    ...
    def ME_scrape(URL):
        ...
        try:
            ...
            for vary in range(0,round(listed/20)+2):
                url = 'https://api-mainnet.magiceden.dev/v2/collections/meerkat_millionaires_country_club/listings?offset={}&limit=20'.format(vary*20)
                page = requests.get(url).json()

                for item in page:
                    price = item['price']
                    data = attys.loc[attys['0'] == item['tokenMint']]
                    id = data['1'].item()
                    seller = 'https://solscan.io/account/' + item['seller']
                    buylink = 'https://www.magiceden.io/item-details/' + item['tokenMint']
                    img = data['2'].item()
                    background = data['7'].item()
                    fur = data['6'].item()
                    earring = data['9'].item()
                    eyes = data['5'].item()
                    glasses = data['10'].item()
                    hat = data['4'].item()
                    mouth = data['8'].item()
                    clothing = data['3'].item()
                    howrare_rank = ranks.loc[ranks['ID'] == int(id), 'HowRare.is'].item()
                    moonrank = ranks.loc[ranks['ID'] == int(id), 'MoonRank'].item()
